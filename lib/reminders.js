...
function reminderLeadLabel(minutes) {
  ...
  if (value % 1440 === 0) return `${value / 1440} ימים לפני`;
  if (value === 60) return "שעה לפני";
  if (value % 60 === 0) return `${value / 60} שעות לפני`;
...
async function sendReminder(chatId, row) {
  const [, date, start, title, minutesBefore] = row;
  const text = [
    `⏰ תזכורת: ${title}`,
    `${formatDateHebrew(date)} · ${start}`,
    `נקבעה ${reminderLeadLabel(minutesBefore)}`,
  ].join("\n");
