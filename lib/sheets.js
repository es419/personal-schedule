...
const REMINDER_HEADERS = ["מזהה אירוע", "תאריך", "התחלה", "אירוע", "דקות לפני", "נשלח ב"];
...
async function readReminderRows(sheets, spreadsheetId) {
  ...
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId,
    range: `'${REMINDER_SHEET_TITLE}'!A2:F`,
  });
  return response.data.values || [];
}

async function upsertReminder(event, sheets, drive, spreadsheetId) {
  const minutesBefore = Number(event.reminderMinutes);
  const rows = await readReminderRows(sheets, spreadsheetId);
  const index = rows.findIndex((row) => row?.[0] === event.id);

  if (!Number.isFinite(minutesBefore) || minutesBefore <= 0) {
    if (index !== -1) {
      await sheets.spreadsheets.values.clear({
        spreadsheetId,
        range: `'${REMINDER_SHEET_TITLE}'!A${index + 2}:F${index + 2}`,
      });
    }
    return;
  }

  const refreshedRows = index === -1 && rows.length === 0 ? [] : await readReminderRows(sheets, spreadsheetId);
  const actualIndex = refreshedRows.findIndex((row) => row?.[0] === event.id);
  const existing = actualIndex !== -1 ? refreshedRows[actualIndex] : null;
  const unchangedSchedule = existing && existing?.[1] === event.date && existing?.[2] === event.start && Number(existing?.[4]) === minutesBefore;
  const sentAt = unchangedSchedule ? existing?.[5] || "" : "";
  const rowNumber = actualIndex === -1 ? refreshedRows.length + 2 : actualIndex + 2;

  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `'${REMINDER_SHEET_TITLE}'!A${rowNumber}:F${rowNumber}`,
    valueInputOption: "RAW",
    requestBody: { values: [[event.id, event.date, event.start, event.title, minutesBefore, sentAt]] },
  });
}

async function removeReminder(id, sheets, spreadsheetId) {
  const rows = await readReminderRows(sheets, spreadsheetId);
...
function reminderMapFromRows(rows) {
  ...
  for (const row of rows || []) {
    ...
    if (row?.[0] && Number.isFinite(minutesBefore) && minutesBefore > 0) {
      map.set(row[0], { reminderMinutes: minutesBefore, reminderSentAt: row?.[5] || "" });
    }
  }
  return map;
}
...
export async function createEvent(event, accessToken) {
  ...
  if (Number(event.reminderMinutes) > 0) {
  ...
  await upsertReminder(event, sheets, drive, spreadsheetId);
  return event;
}
...
export async function updateEvent(id, previousDate, event, accessToken) {
  ...
  await upsertReminder(event, found.sheets, drive, found.spreadsheetId);
  return event;
}
...
export async function deleteEvent(id, dateString, accessToken) {
  ...
  await removeReminder(id, found.sheets, found.spreadsheetId);
  const startColumn = columnName(found.dayIndex * COLS_PER_DAY);
  const endColumn = columnName(found.dayIndex * COLS_PER_DAY + COLS_PER_DAY - 1);
